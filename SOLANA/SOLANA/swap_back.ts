...
//   const inputMint = 'EATGZHJViJsk7nEKkrdJicwNbfpkJfAtmrEmrjXR8NBj'
...
export const apiSwap = async () => {
  // const inputMintAddress = 'EATGZHJViJsk7nEKkrdJicwNbfpkJfAtmrEmrjXR8NBj'; // Token mint address (for your meme token)
  const inputMintAddress = 'FADUeDwhueWsCEQNXcSiAJeFa5yyJffgKZmQWY1yQ9rw'; // Token mint address (for your meme token)
  const outputMint = NATIVE_MINT.toBase58(); // SOL

  // Fetch the mint info to get the decimals for the input token
  ...
  console.log(`Swapping ${amountInTokens} tokens (in smallest units: ${amount}) of token with mint ${inputMint.toBase58()}`);

  const slippage = 2.0; // in percent, for this example, 2.0 means 2%
  const txVersion: string = 'V0'; // or LEGACY
  const isV0Tx = txVersion === 'V0';

  const [isInputSol, isOutputSol] = [inputMintAddress === NATIVE_MINT.toBase58(), outputMint === NATIVE_MINT.toBase58()];

  const { tokenAccounts } = await fetchTokenAccountData();
  const inputTokenAcc = tokenAccounts.find((a) => a.mint.toBase58() === inputMintAddress)?.publicKey;
  const outputTokenAcc = tokenAccounts.find((a) => a.mint.toBase58() === outputMint)?.publicKey;
